    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-6">
        <div className="p-2 bg-blue-100 rounded-lg mr-3 w-10 h-10 flex items-center justify-center">
          <Search className="h-5 w-5 text-[var(--theme-color-1)]" />
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
        ...
        <div className="relative">
          <label htmlFor="city" className="flex items-center text-sm font-medium text-gray-700 mb-1.5">
            <MapPin className="w-4 h-4 mr-1.5 text-[var(--theme-color-1)]" />
            City
          </label>
          <div className="relative">
            <input
              ...
              className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--theme-hover-color-1)] focus:border-[var(--theme-hover-color-1)] focus:outline-none transition-shadow text-sm sm:text-base"
              required
            />
            {activeField === 'city' && (
              <motion.div 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div>
            <label htmlFor="maxPrice" className="flex items-center text-sm font-medium text-gray-700 mb-1.5">
              <IndianRupee className="w-4 h-4 mr-1.5 text-[var(--theme-color-1)]" />
          ...
          <div>
            <label htmlFor="propertyType" className="flex items-center text-sm font-medium text-gray-700 mb-1.5">
              <Home className="w-4 h-4 mr-1.5 text-[var(--theme-color-1)]" />
              Property Type
            </label>
            <select
              id="propertyType"
              ...
              onChange={handleChange}
              className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--theme-hover-color-1)] focus:border-[var(--theme-hover-color-1)] focus:outline-none transition-shadow appearance-none text-sm sm:text-base"
            >
          ...
          <div>
            <label htmlFor="propertyCategory" className="flex items-center text-sm font-medium text-gray-700 mb-1.5">
              <Building className="w-4 h-4 mr-1.5 text-[var(--theme-color-1)]" />
              Property Category
            </label>
            <select
              id="propertyCategory"
          ...
          <div>
            ...
            <input
              ...
              className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[var(--theme-color-1)]"
            />
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.99 }}
          disabled={isLoading}
          className="w-full mt-2 sm:mt-4 bg-gradient-to-r from-[var(--theme-color-1)] to-[var(--theme-hover-color-1)] text-white py-3 sm:py-4 px-4 sm:px-6 rounded-lg hover:from-[var(--theme-hover-color-1)] hover:to--700 focus:outline-none focus:ring-2 focus:ring-[var(--theme-hover-color-1)] focus:ring-offset-2 transition-all duration-300 font-medium shadow-lg disabled:opacity-70"
        >
          {isLoading ? (
